...
const pivotToggleParams = { EnablePivotControl: false };
    gui.add(pivotToggleParams, "EnablePivotControl")
    ...
    .onChange((value) => {
        ...
        pivotFolder.domElement.style.pointerEvents = isPivotEnabled ? "auto" : "none";
    ...
    gui.add(cubeVisibilityParams, "ShowCube")
        .name("Show Cube")
        .onChange((value) => {
    ...
    positionFolder.add(savedYParams, "UseSavedY")
    ...
    // "발을 땅에 붙이기" 버튼 추가
    const attachToGroundController = positionFolder.add({ AttachToGround: () => attachToGround() }, "AttachToGround").name("발을 땅에 붙이기");

    // 버튼 스타일 수정
    setTimeout(() => {
    const buttonElement = attachToGroundController.domElement.querySelector("button");
    if (buttonElement) {
...
zoomCtrl = cameraFolder.add(cameraParams, "zoom", 0.1, 5).onChange((value) => {
    camera.zoom = value;
    camera.updateProjectionMatrix();
});
...
lightXCtrl = lightFolder.add(lightParams, "lightX", -500, 500).onChange((value) => {
    dirLight.position.x = value;
});
lightYCtrl = lightFolder.add(lightParams, "lightY", 0, 500).onChange((value) => {
    dirLight.position.y = value;
});
lightZCtrl = lightFolder.add(lightParams, "lightZ", -500, 500).onChange((value) => {
...
pivotXCtrl = pivotFolder.add(pivotParams, "pivotX", -500, 500).onChange((value) => {
    if (!isPivotEnabled) return; // 피벗 조작 비활성화 시 동작 중단
    const delta = value - pivot.position.x;
    pivot.position.x = value;

    const worldDelta = new THREE.Vector3(delta, 0, 0).applyQuaternion(pivot.quaternion);
    model.position.sub(worldDelta);
});

pivotYCtrl = pivotFolder.add(pivotParams, "pivotY", -500, 500).onChange((value) => {
    if (!isPivotEnabled) return;
    const delta = value - pivot.position.y;
    pivot.position.y = value;

    const worldDelta = new THREE.Vector3(0, delta, 0).applyQuaternion(pivot.quaternion);
    model.position.sub(worldDelta);
});

pivotZCtrl = pivotFolder.add(pivotParams, "pivotZ", -500, 500).onChange((value) => {
    if (!isPivotEnabled) return;
    const delta = value - pivot.position.z;
    pivot.position.z = value;

    const worldDelta = new THREE.Vector3(0, 0, delta).applyQuaternion(pivot.quaternion);
    model.position.sub(worldDelta);
});
...
pivotFolder.add({ Unlock: false }, "Unlock")
...
visualizationFolder
    ...
    const { r, g, b } = hsvToRgb(colorParams.hue, colorParams.saturation, colorParams.value);
